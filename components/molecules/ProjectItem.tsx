...
function ProjectRow({ project }: { project: Project }) {
  ...
  return (
    <div className="row-item" style={{ position: 'relative', padding: '24px 4px' }}>
      <Link href={href} aria-label={project.title} style={{ position: 'absolute', inset: 0, zIndex: 0 }} />
      <div className="row-stack" style={{ position: 'relative', pointerEvents: 'none' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 8, pointerEvents: 'auto' }}>
            {project.link && (
              <a href={project.link} target="_blank" rel="noopener noreferrer" style={actionBtnStyle}>
                <IcoLink /> Link
              </a>
            )}
            {project.source && (
              <a href={project.source} target="_blank" rel="noopener noreferrer" style={actionBtnStyle}>
